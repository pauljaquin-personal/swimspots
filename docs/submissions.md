...
5. Deploy the reviewed branch when authorised, verify unauthorized `/api/review` returns 401, submit a controlled test record, review it and confirm only its public fields appear on the map.
...
- Server validation, 16 KB payload limit, required JSON and exact same-origin writes.
...
- Published records cannot yet be edited or withdrawn in this review UI; corrections currently require an administrator database update. Keep the first release in controlled curation until that workflow is added. Contributor accounts, photographs, notifications and reputation are later work.
